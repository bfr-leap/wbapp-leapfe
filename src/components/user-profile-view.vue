...
let userProfileModel: Ref<UserProfileModel> = ref(
...
async function onSubmitCustId(event: MouseEvent) {
    userProfileModel.value.enableCustIdSendButton = userProfileModel.value.enableVerifySendButton = false;
    userProfileModel.value = await sendCustId(forms.custId);
}

async function onSubmitVerificationNum(event: MouseEvent) {
    userProfileModel.value.enableCustIdSendButton = userProfileModel.value.enableVerifySendButton = false;
    userProfileModel.value = await sendVerification(Number.parseInt(forms.verificationNum));
}
        ...
        <div v-if="userProfileModel.isVerified === false" class="card bg-dark text-light m-2">
            <div class="card-body p-2">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-auto">
                        <button @click="onSubmitCustId"
                            v-if="userProfileModel.enableCustIdSendButton && userProfileModel.irCustId !== forms.custId"
                            type="submit" class="btn btn-primary">Submit</button>
                        <button v-else type="submit" class="btn btn-primary" disabled>Submit</button>

                    </div>
                    <div class="col-auto">
                        <div id="custIdHelp" class="form-text">We'll send you an iRacing PM</div>
                    </div>
                </div>
                ...
                <div v-if="userProfileModel.msgSent" class="row g-3">
                    ...
                    <div class="col-auto">
                        <button @click="onSubmitVerificationNum"
                            v-if="userProfileModel.enableVerifySendButton && forms.verificationNum" type="submit"
